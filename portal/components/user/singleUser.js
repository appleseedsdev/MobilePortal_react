apple.controller('singleUser', ['$rootScope', '$scope', '$state', '$stateParams', '$http','userService','Upload', 'server', '$q', function ($rootScope, $scope, $state, $stateParams, $http, userService, Upload, server, $q) {
   ...
   //  console.log(server);
    ...
    $scope.getEnrolledInCoursesInRole = function(roleid) {
        $scope.user.enrolledAsTeacher=[];
        $scope.user.enrolledAsStudent=[];
        ...
        server.requestPhp(data, 'GetCoursesWithUserEnrolledAsRole').then(function (data) {
         //   console.log(data);
            if(roleid=='1')
            {
                ...
                {
                    $scope.user.enrolledAsTeacher.push(data.enrolled[i].courseid);
                }
            }
            else if(roleid=='2')
            {
                for(var i=0;i<data.enrolled.length;i++)
                {
                    $scope.user.enrolledAsStudent.push(data.enrolled[i].courseid);
                }
            }
    ...
    $scope.getEnrolledInCoursesInRoleFull = function(paramObj) {
        ...
        server.requestPhp(data, 'GetCoursesWithUserEnrolledAsRole').then(function (data) {
            courses = data;
            var header = {coursename:"קורס", projectname: "פרוייקט",code:"קוד קורס"};
    ...
    $scope.GetProfessions = function () {
        var data = {};
        server.requestPhp(data, 'GetActiveProfessions').then(function (data) {
    ...
    $scope.GetCertificates = function () {
        var data = {};
        server.requestPhp(data, 'GetActiveCertificates').then(function (data) {
    ...
    $scope.searchProjects = function (search, page, onSuccess) {
        var desc = false;
        var data ={'search': search, 'page': page, 'userid':$scope.userId};
    ...
    var PopAllProjectsIds = function() {
        ...
        for(var j=0;j<$scope.Userprojects.length;j++) {
            $scope.user.projects.push($scope.Userprojects[j].projectid);
        }
    };
    ...
    var UpdateUserProjects = function()
    {
        var data = {};
        data.userid = $scope.userId;
        data.projectsids = $scope.user.projects;
        server.requestPhp(data, 'UpdateUserProjects').then(function (data) {
        });
    }
    ...
    $scope.removeProject = function(project){
        for(var i=0;i<$scope.Userprojects.length;i++)
        {
            if($scope.Userprojects[i]===project) {
                ...
                $scope.user.projects.splice( $scope.user.projects.indexOf(project.projectid),1);
            }
        }
