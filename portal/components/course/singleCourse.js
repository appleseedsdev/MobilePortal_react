...
function($rootScope, $scope, $state, $stateParams, $http, $q, userService, Upload, server) {
    ...
    var GetLessonsOfCourse = function() {
        var data ={};
        data.courseid=$scope.courseid;
        server.requestPhp(data, 'GetLessonsOfCourse'
        ).then(function (data) {
            $scope.Lessons = data;

            for(var i=0;i<$scope.Lessons.length;i++)
			{
				var holder=$scope.Lessons[i].beginningdate;
				var date = new Date(parseInt(holder));
                $scope.Lessons[i].date =  moment(date).format('DD/MM/YY');
                $scope.Lessons[i].times = moment(date).format('HH:mm');
			}
        });
    };
	...
	var getBudgetYears = function() {
		var data={};
		server.requestPhp(data, 'GetActiveYearsBudget').then(function (data) {
   ...
   // GetEnrollmentTagsForTeachers();
	...
	$scope.getEnrolledUsersInRole = function(roleid) {
		if($scope.displayedEnrollments.length>0)
		{
	...
	$scope.getEnrolledUsersInRoleFull=function(paramObj) {
		...
		server.requestPhp(data, 'GetCourseEnrollmentProfiles').then(function (data) {
			...
			users.forEach(function(el){
				...
				{
					//el.birthday=moment(el.birthday,"DD-MM-YYYY")._d;
                    var holder=el.birthday;
                    var date = new Date(parseInt(holder));
                    el.birthday=moment(date).format('DD/MM/YYYY')._d;
                  //  console.log(  el.birthday);
				}
			});
